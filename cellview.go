...
type Char struct {
	visualLoc Loc
	realLoc   Loc // how to get real location
	char      rune
...
func (c *CellView) Draw(buf *Buffer, top, height, left, width int) {
	...
	if buf.Settings["matchbrace"].(bool) {
		for _, bp := range bracePairs {
			curX := buf.Cursor.X
			curLoc := buf.Cursor.Loc // get cursor location
	...
	c.lines = make([][]*Char, 0) // cellview lines
	...
	for viewLine < height {
		...
		lineStr := buf.Line(lineN) // get the linestring at line lineN
		line := []rune(lineStr)
