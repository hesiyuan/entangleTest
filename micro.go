...
func main() {
	...
	go func() { // currently it works, but updates so quickly
		var x, y int
		// lineStyle := defStyle.Reverse(true)
		// if style, ok := colorscheme["selection"]; ok { // cursor style
		// 	lineStyle = style
		// }
		...
		for {
			//screen.SetContent(5, 5, ' ', nil, defStyle)
			//screen.Show() // this affects the whole screen
			//m.Display() // displays the message
			//time.Sleep(500 * time.Millisecond)
			//screen.ShowCursor(5, 5)
			...
			// 					style := GetColor("cursor-line")
			// fg, _, _ := style.Decompose()
			// st := defStyle.Background(fg) // This is important
			...
			x, y = peerCursors[0].X, peerCursors[0].Y
			// This is tricky, because the content might be replaced by insert/deletes...
			//char, combc, style, _ := screen.GetContent(x+2, y) // saved the copy
			char, _, _, _ := screen.GetContent(x+2, y) // saved the copy
			screen.SetContent(x+2, y, char, nil, defStyle.Reverse(true))
			screen.Show()
			time.Sleep(1000 * time.Millisecond)
			// this is trouble some, how about getting from lineArray directly
			//visualToCharPos(x+2, y ,int(buf.Settings["tabsize"].(float64)))
			// char, combc, _, _ := screen.GetContent(x+2, y) // saved the copy
			// screen.SetContent(x+2, y, char, combc, defStyle)
			// screen.Show()
			//RedrawAll()
			//time.Sleep(500 * time.Millisecond)
		}
