...
func (v *View) DisplayView() {
	...
	for visualLineN, line = range v.cellview.lines {
		...
		var realLoc Loc
		var visualLoc Loc
		var cx, cy int // conversion from realLoc and visualLoc is non-trivial
		if lastChar != nil {
			...
			v.SetCursor(&v.Buf.Cursor)
			realLoc = Loc{lastChar.realLoc.X + 1, realLineN} // depends
...
func ShowMultiCursor(x, y, i int) {
	...
	} else { // drawing a fake cursor, yep..
		r, _, _, _ := screen.GetContent(x, y)
		screen.SetContent(x, y, r, nil, defStyle.Reverse(true)) // what's reverse?
	}
}
